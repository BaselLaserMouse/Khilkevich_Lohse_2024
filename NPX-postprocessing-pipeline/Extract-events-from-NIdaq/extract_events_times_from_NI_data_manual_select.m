function extract_events_times_from_NI_data_manual_select
...
ind_tmp = strfind(binName, '_g0');
session_name = binName(1:ind_tmp-1);
...
[NIdaq_events.Front_cam.rise_t, NIdaq_events.Front_cam.fall_t, NIdaq_events.Cam1.duration] = get_event_times_threshold(digArray(2, :), 'D', [], NI_sample_rate);        
[NIdaq_events.Eye_cam.rise_t, NIdaq_events.Eye_cam.fall_t, NIdaq_events.Cam2.duration] = get_event_times_threshold(digArray(3, :), 'D', [], NI_sample_rate);
[NIdaq_events.Top_cam.rise_t, NIdaq_events.Top_cam.fall_t, NIdaq_events.Cam3.duration] = get_event_times_threshold(digArray(4, :), 'D', [], NI_sample_rate);
...
threshold_gen = 0.5; %in V, use for step-wise event types
threshold_lick = 0.15;
...
Baseline_ON_min_dur = 0.02; %20ms, typical length of split is 5ms
splits_tr_ind = find(Baseline_ON_dur<Baseline_ON_min_dur);
...
disp(['Expected number of trials: ' num2str(length(Baseline_ON_new.duration))])    

NIdaq_events.Baseline_ON = Baseline_ON_new;
%% Use cleaned Baseline_ON events to check Change_ON signal for splits too

ChangeON_splits_numb = 0;

Change_ON_rise_t_new = [];
Change_ON_fall_t_new = [];
Change_ON_duration_new = [];

trials_numb = length(NIdaq_events.Baseline_ON.rise_t);

for tr = 1:trials_numb
    
    Baseline_end_tr = NIdaq_events.Baseline_ON.fall_t(tr);
    Change_ON_start_ind_tr = find( (NIdaq_events.Change_ON.rise_t >= (Baseline_end_tr - 0.1) )&(NIdaq_events.Change_ON.rise_t < Baseline_end_tr+3) );
    
    if length(Change_ON_start_ind_tr)>1     % find splits in ChangeON (more then 1 event per trial) 
        
        ChangeON_splits_numb = ChangeON_splits_numb + length(Change_ON_start_ind_tr) - 1;
        Change_ON_start_tr = NIdaq_events.Change_ON.rise_t(Change_ON_start_ind_tr);
        Change_ON_end_tr = NIdaq_events.Change_ON.fall_t(Change_ON_start_ind_tr);
        IEI = Change_ON_start_tr(2:end) - Change_ON_end_tr(1:end-1);
        
        if  max(IEI)<0.01     % stitch events if separation is less then 10 ms
            Change_ON_rise_t_new{tr, 1} = NIdaq_events.Change_ON.rise_t( Change_ON_start_ind_tr(1) );
            Change_ON_fall_t_new{tr, 1} = NIdaq_events.Change_ON.fall_t( Change_ON_start_ind_tr(end) );
            Change_ON_duration_new{tr, 1} = Change_ON_fall_t_new{tr, 1} - Change_ON_rise_t_new{tr, 1};
        else
            disp('Found splits of ChangeON, but over 10 ms apart')
        end       
        
    else
        if isempty(Change_ON_start_ind_tr)
%             disp(['No ChangeON event on trial ' num2str(tr)])
            Change_ON_rise_t_new{tr, 1} = []; 
            Change_ON_fall_t_new{tr, 1} = []; 
            Change_ON_duration_new{tr, 1} = [];
        else
            Change_ON_rise_t_new{tr, 1} = NIdaq_events.Change_ON.rise_t( Change_ON_start_ind_tr );  
            Change_ON_fall_t_new{tr, 1} = NIdaq_events.Change_ON.fall_t( Change_ON_start_ind_tr );  
            Change_ON_duration_new{tr, 1} = NIdaq_events.Change_ON.duration( Change_ON_start_ind_tr );  
        end
    end
end

NIdaq_events.Change_ON.rise_t = Change_ON_rise_t_new; 
NIdaq_events.Change_ON.fall_t = Change_ON_fall_t_new;
NIdaq_events.Change_ON.duration = Change_ON_duration_new;

disp(['Corrected ' num2str(ChangeON_splits_numb) ' ChangeON events'])
disp(['Expected number of ChangeON events: ' num2str(sum( cell2mat(Change_ON_duration_new(:))>0) )])    
...
for tr = 1:length(Baseline_ON_smpl_ind)             % get readings of high/low states in photodiode 
    ...
    photodiode_trial_sorted = sort( photodiode_ch( Baseline_ON_smpl_ind(tr) : Baseline_ON_smpl_ind(tr)  + round(NI_sample_rate)), 'descend' );
...
if (max(photodiode_pre_trial_mean)>min(photodiode_trial_high)) || ( min(photodiode_pre_trial_mean)<max(photodiode_trial_low) )  % check if photodiodfe is consistent and usable 
    ...
    [~, up_state_times] = findpeaks(photodiode_ch, t, 'MinPeakProminence', (median(photodiode_trial_high)- mean(photodiode_pre_trial_mean) - 0.1), 'MinPeakDistance', 0.01, 'MinPeakHeight', 0.5);   
    % find local minima in photodiode trace
    inv_photodiode_ch_zero_mean = -(photodiode_ch - mean(photodiode_pre_trial_mean) );
    
    [~, low_state_times] = findpeaks( inv_photodiode_ch_zero_mean, t, 'MinPeakProminence', mean(photodiode_pre_trial_mean) - (median(photodiode_trial_low)-0.1), 'MinPeakDistance', 0.01, 'MinPeakHeight', 0.5);
...
for tr = 1:trials_numb
    ...
    if isempty(tr_end_time) % no Change event
...
save(fullfile('D:\NIdaq_events_data', [session_name '_NIdaq_events.mat']), 'NIdaq_events');
