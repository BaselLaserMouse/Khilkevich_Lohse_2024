...
for curClu=GoodClustersToRun%1:length(EphysData.Clu)
    ...
    cd([MetaData.ResultsFolder '/ReducedPreLick'])
    
    if isfile(['ReducedPreLick-'  RegionName '-' SubjectIds{MetaData.SubId} '-' SessionSelected.behav_data.SessionSettings.token '-ProbeNo_' num2str(MetaData.ProbeNo) '-CluId_' num2str(MetaData.CluId) '-GoodCluNo_' num2str(curClu) '.mat'])
        toc
    ...
    if contains( MetaData.Region , '/' )
        backslash_pos=strfind( MetaData.Region , '/' );
        RegionName=MetaData.Region(setdiff(1:length( MetaData.Region),backslash_pos));
        FileSaveName=['ReducedPreLick-'  RegionName '-' SubjectIds{MetaData.SubId} '-' SessionSelected.behav_data.SessionSettings.token '-ProbeNo_' num2str(MetaData.ProbeNo) '-CluId_' num2str(MetaData.CluId) '-GoodCluNo_' num2str(curClu)];
    else
        FileSaveName=['ReducedPreLick-'  MetaData.Region '-' SubjectIds{MetaData.SubId} '-' SessionSelected.behav_data.SessionSettings.token '-ProbeNo_' num2str(MetaData.ProbeNo) '-CluId_' num2str(MetaData.CluId) '-GoodCluNo_' num2str(curClu)];
    end
