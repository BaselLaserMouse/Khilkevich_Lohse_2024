...
%% Baseline only model
PredictorParams.BaselineOnly=1; % if you choose this keep BaseOn, Lick, Airpuff, Abort and Rew on, as these will be used to fully remove these preiods when this is set to 1

%% Full model parameters
PredictorParams.inclSpikeHist=0;
PredictorParams.inclTFfilter=1;
PredictorParams.Separate_SlowFast_TFFilters=1;
...
PredictorParams.inclPostLick=1;
PredictorParams.inclAirpuff=0;
PredictorParams.inclRew=1;
PredictorParams.inclAbort=0;
...
PredictorParams.FaceMovement=1; %
PredictorParams.Pupil=1; %
PredictorParams.RunWheel=1; %
...
for curClu=GoodClustersToRun%1:length(EphysData.Clu)
    ...
    cd([MetaData.ResultsFolder '/BaselineGLM'])
    
    if isfile(['BaselineGLM-'  RegionName '-' SubjectIds{MetaData.SubId} '-' SessionSelected.behav_data.SessionSettings.token '-ProbeNo_' num2str(MetaData.ProbeNo) '-CluId_' num2str(MetaData.CluId) '-GoodCluNo_' num2str(curClu) '.mat'])
        toc
    ...
    [rawData.trial,rawData.param]=MakeTFCDEventGLMInputStructureFullBlocksNaive(EphysData,BehavData,VideoData,MetaData.HitTrials,MetaData.nTrials,curClu,PredictorParams.binSize,MetaData.ExpName,MetaData.ProbeNo,PredictorParams.onlyHit,PredictorParams.excludeMiss);
    ...
    if contains( MetaData.Region , '/' )
        backslash_pos=strfind( MetaData.Region , '/' );
        RegionName=MetaData.Region(setdiff(1:length( MetaData.Region),backslash_pos));
        FileSaveName=['BaselineGLM-'  RegionName '-' SubjectIds{MetaData.SubId} '-' SessionSelected.behav_data.SessionSettings.token '-ProbeNo_' num2str(MetaData.ProbeNo) '-CluId_' num2str(MetaData.CluId) '-GoodCluNo_' num2str(curClu)];
    else
        FileSaveName=['BaselineGLM-'  MetaData.Region '-' SubjectIds{MetaData.SubId} '-' SessionSelected.behav_data.SessionSettings.token '-ProbeNo_' num2str(MetaData.ProbeNo) '-CluId_' num2str(MetaData.CluId) '-GoodCluNo_' num2str(curClu)];
    end
