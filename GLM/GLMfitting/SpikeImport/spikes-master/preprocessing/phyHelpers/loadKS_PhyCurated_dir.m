...
function spikeStruct = loadKSdir(ksDir,qual_thresh, varargin)
...
if exist(fullfile(ksDir, 'cluster_group.tsv')) 
   cgsFile = fullfile(ksDir, 'cluster_group.tsv');
    ...
    qualFile = fullfile(ksDir, 'cluster_qual.tsv'); % load quality assessment

% if ~isempty(cgsFile)
    [cids, cgs] = readClusterGroupsCSV(cgsFile);
    [qids, qgs] = readClusterQualCSV(qualFile);

    if params.excludeNoise
        noiseClusters = cids(cgs==0);

        st = st([~ismember(clu, noiseClusters) + ismember(clu, qids(find(qgs<=qual_thresh)))]==2);
        ...
        clu = clu([~ismember(clu, noiseClusters) + ismember(clu, qids(find(qgs<=qual_thresh)))]==2); % incuded cl has to meet all three criteria
        
        GoodClu=find(qgs<=qual_thresh);

        qgs = qgs([ismember(qids,qids(GoodClu)) + ~ismember(qids, noiseClusters)]==2);
        qids = qids([ismember(qids,qids(GoodClu)) + ~ismember(qids, noiseClusters)]==2);
    end
