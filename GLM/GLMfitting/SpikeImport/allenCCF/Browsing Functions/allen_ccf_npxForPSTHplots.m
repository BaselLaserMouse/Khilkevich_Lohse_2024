function allen_ccf_npxForPSTH(tv,av,st,ProbePos,AnglesUsed)
...
% Find distance to brain surface at penetratio site
DisttoSurf=find(av(bregma(1)-(ProbePos(1)/10),:,bregma(3)+(ProbePos(2)/10))>1,1,'first')
%Position probe
probe_ref_top = [bregma(1)-(ProbePos(1)/10),bregma(3)+(ProbePos(2)/10),DisttoSurf];
probe_ref_bottom = [bregma(1)-(ProbePos(1)/10),bregma(3)+(ProbePos(2)/10),size(tv,2)];
...
probe_angle_rad = (AnglesUsed./360)*2*pi;
[x,y,z] = sph2cart(pi-probe_angle_rad(1),probe_angle_rad(2),max_ref_length);

new_probe_ref_top = [probe_ref_vector(1,1),probe_ref_vector(2,1),DisttoSurf];
...
probe_vector = [new_probe_ref_vector(:,1),diff(new_probe_ref_vector,[],2)./ ...
    norm(diff(new_probe_ref_vector,[],2))*probe_length + new_probe_ref_vector(:,1)];

% plot probe
probe_ref_line = line(new_probe_ref_vector(1,:),new_probe_ref_vector(2,:),new_probe_ref_vector(3,:), ...
    'linewidth',1.5,'color','r','linestyle','--');
...
gui_data.probe_angle = AnglesUsed; % Probe angles in ML/DV
