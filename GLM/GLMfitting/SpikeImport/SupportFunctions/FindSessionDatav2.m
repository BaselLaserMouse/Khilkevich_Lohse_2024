...
if Runs == 1
    ...
    for T=1:length(TrialData)
        HitTrials(T)=TrialData{1}.trialoutcome(1)=='H'; % TODO: double check if there are other otcomes that start with H
        RT(T)=TrialData{1}.reactiontimes.RT; 
        BaseTFvals{T}=TrialData{1}.St1TrialVector(1:3:floor(TrialData{1}.stimT/(0.1/6))); % Finding ales for each of the drfting speeds (TF) assuming each TF value is 3 frames lng with a 16.66 ms refrs rate 
        BaseTime(T)=TrialData{1}.stimT; % Finding ales for each of the drfting speeds (TF) assuming each TF value is 3 frames lng with a 16.66 ms refrs rate 
