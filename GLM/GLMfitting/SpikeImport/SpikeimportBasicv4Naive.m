function [Expt]=SpikeImportBasicv4(Session,ProbeNo,Window,BinSize,Smoothing,GoodClustersToInclude)

% v4 only GLM extraction
...
for e=1%:length(DataFolders) % experiment (penetration/or simulanouspenetrations)
    ...
    Expt(e).AllEventTimes=FindEventsfromNPXv4Naive(Session.NI_events);
    
    Expt.nTrials=length(Expt(e).AllEventTimes{2});
    
    % Find Session and Trial Data
    disp('Finding session and trial data')
    [Expt(e).HitTrials, ~,Expt(e).BaseTFvals,Expt(e).BaseOri,Expt(e).BaseTime,Expt(e).FastPulseTimes,Expt(e).SlowPulseTimes]=FindSessionDatav4(Session.behav_data); % fodler with ephys data and amount of runs (usually only one, but in case there was a restart this can be changed) for this session
    
    NumCluTotal=length(unique(Session.NPX_probes(ProbeNo).clu)); % number of cluster in current penetration
    NumClu=length(Session.NPX_probes(ProbeNo).cluster_id_good_and_stable); % number of cluster in current penetration
    
    disp([num2str(NumCluTotal) ' Total clusters in current penetration'])
    disp([num2str(NumClu) ' Good and stable clusters in current penetration'])
    
    for CluCounter=GoodClustersToInclude
        
        curClu=Session.NPX_probes(ProbeNo).cluster_id_good_and_stable(CluCounter);
