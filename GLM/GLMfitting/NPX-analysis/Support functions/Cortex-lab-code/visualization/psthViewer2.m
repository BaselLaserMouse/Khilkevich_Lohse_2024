...
function [psthSm, bins] = psthViewer2(spikeTimes, clu, eventTimes, window, trGroups, cluster_depth, gauss_st, conf_interv)
...
params.smoothSize = gauss_st; % in msec, stdev of gaussian smoothing filter
params.clusterIndex = 1;
params.rasterScale = floor(numel(eventTimes)/100); % height of ticks
params.window_orig = window;
...
myData.clusteryCoords = cluster_depth;
myData.trGroupLabels = unique(myData.trGroups);
myData.nGroups = length(myData.trGroupLabels);
myData.plotAxes = [];
myData.use_conf_interv = conf_interv;

if myData.nGroups>4
    params.colors = lines(myData.nGroups); 
else
%     params.colors = ['k' ;'b' ; 'r'; 'g'];
    params.colors = ['k' ; 'r'; 'b'; 'g'];

end
% params.colors = [1 0 0; 0 0 0];
...
use_conf_interv = myData.use_conf_interv;
...
if myData.params.showAllTraces
    for g = 1:nGroups
        ...
        hold on;
        [~, ~, fr_conf, ~] = normfit(baSm(myData.trGroups==trGroupLabels(g),:));
%         fr_conf = bootci(10,{@(x) mean(x), baSm(myData.trGroups==trGroupLabels(g),:)},'alpha',0.05);
        if use_conf_interv ==1
            ciplot(fr_conf(1,:), fr_conf(2,:), bins,  colors(g,:), 0.25);
        end
    end
else
    plot(bins, psthSm, 'k');
    hold on
    [~, ~, fr_conf, ~] = normfit(baSm);
%     fr_conf = bootci(100,{@(x) mean(x), baSm},'alpha',0.05);
    if use_conf_interv ==1
        ciplot(fr_conf(1,:), fr_conf(2,:), bins, 'k', 0.25);
    end
end

xlim(myData.params.window_orig);
title(['cluster ' num2str(myData.clusterIDs(myData.params.clusterIndex)),' - Depth: ' num2str(myData.clusteryCoords(myData.params.clusterIndex))]);
xlabel('Time, s');
