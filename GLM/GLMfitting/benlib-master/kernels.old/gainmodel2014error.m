function  [E, dE] = gainmodel2014error(X, fitdata)
...
% fitdata.y_t = rand(200,1); fitdata.z_t = rand(200,1); fitdata.C_t = randi([0 1], [200,1]);
% d = checkgrad('gainmodel2014error', rand(6,1), 1e-5, fitdata)
%
...
% Making exp(-(z_t-cp)/dp) (recurs in partials)
x = -(z_t-cp)./dp;
exp_x = exp(x);
...
% dfX/da = 1
dE(1) = sum(2*residuals); % unchanged from simple LN model

% dfX/db = 1./(1+exp_x)
dE(2) = sum(2*residuals ./ (1+exp_x)); % exp_x has changed; otherwise same as simple LN model

% dfX/dc = -b*exp_x /[d*(1+exp_x)^2]
%        = -b /[d*(1+exp_x)] * etrat
%dE(3) = sum(2*residuals * -b ./ (d*(1+exp_x)) .* etrat);
dE(3) = sum(2*residuals * -b ./ (dp.*(1+exp_x)) .* etrat); % substitute dp for d

% dfX/dd = b(-z_t+c)*exp_x / [d*(1+exp_x)]^2
%        = b(-z_t+c)/[d^2*(1+exp_x)] * etrat
dE(4) = sum(2*residuals *b.*(-z_t+cp)./((dp.^2).*(1+exp_x)) .* etrat); % substtitude cp, dp for c, d

dE(5) = sum(-(2*b.*C_t.*etrat.*residuals)./(dp.*(1+exp_x)));

dE(6) = sum((2*b.*C_t.*(cp-z_t).*etrat.*residuals)./((dp.^2).*((1+exp_x))));
