function [E, dE] = gainmodelSSE_partials(x, data)
% function y_hat_t = gainmodelSSE_partials(x, data)
%
...
% d = checkgrad('gainmodelSSE_partials', rand(6,1), 1e-5, fitdata)
...
% dfX/db = 1./(1+exp_x)
dE(2) = sum(2 .* residuals ./ (1+(exp_x_L.*(C_t==0))+(exp_x_H.*(C_t==1))));

% dfX/dc = -b*exp_x /[d*(1+exp_x)^2]
%        = -b /[d*(1+exp_x) * etrat]
dE(3) = sum(2 .* residuals .* -b .* (C_t==0) .* etrat_L_sq ./ d_L);

% dfX/dc = -b*exp_x /[d*(1+exp_x)^2]
%        = -b /[d*(1+exp_x)] * etrat
dE(4) = sum(2 .* residuals .* -b .* (C_t==1) .* etrat_H_sq ./ d_H);;

% dfX/dd = b(-z_t+c)*exp_x / [d*(1+exp_x)]^2
%        = b(-z_t+c)/[d^2*(1+exp_x)] * etrat
dE(5) = sum(2 .* residuals .* b .* (C_t==0) .* (-z_t+c_L) .* etrat_L_sq ./ d_L.^2);

% dfX/dd = b(-z_t+c)*exp_x / [d*(1+exp_x)]^2
%        = b(-z_t+c)/[d^2*(1+exp_x)] * etrat
dE(6) = sum(2 .* residuals .* b .* (C_t==1) .* (-z_t+c_H) .* etrat_H_sq ./ d_H.^2);
