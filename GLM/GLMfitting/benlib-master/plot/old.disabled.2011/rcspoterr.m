...
%
function smap = rcspoterr(pypefile, varargin)
...
hfhimat=zeros(ycount,xcount,lagcount,trialcount)+nan;
hflomat=zeros(ycount,xcount,lagcount,trialcount)+nan;
...
for tridx=1:trialcount,
   ...
   for ii=1:size(events,1),
	 ...
	 hflomat(iy,ix,:,nlo(iy,ix)) = histspikes;
      else
         hfhi(iy,ix,:)=hfhi(iy,ix,:)+histspikes;
         nhi(iy,ix)=nhi(iy,ix)+1;
	 hfhimat(iy,ix,:,nhi(iy,ix)) = histspikes;
      end
...
fprintf('\n');
fprintf([num2str(min([nlo(:); nhi(:)])) ' completed reps\n']);
...
hflomat=hflomat./binsize*1000;
hfhimat=hfhimat./binsize*1000;

% get mean
hflomat_tmp = hflomat;
hflomat_tmp(find(isnan(hflomat)))=0;
hflo2 = sum(hflomat_tmp,4); 
hflo2 = hflo2./repmat(nlo,[1 1 lagcount]); % should be equal to hflo

% get sd
sqdev = (hflomat-repmat(hflo,[1 1 1 trialcount])).^2;
sqdev(find(isnan(sqdev))) = 0;
ssdev = sum(sqdev,4);
rmsdev = sqrt(ssdev./repmat(nlo,[1 1 lagcount])); 
hflosem = (rmsdev./repmat(sqrt(nlo),[1 1 lagcount]));

% get mean
hfhimat_tmp = hfhimat;
hfhimat_tmp(find(isnan(hfhimat)))=0;
hfhi2 = sum(hfhimat_tmp,4); 
hfhi2 = hfhi2./repmat(nhi,[1 1 lagcount]); % should be equal to hfhi

% get sd
sqdev = (hfhimat-repmat(hfhi,[1 1 1 trialcount])).^2;
sqdev(find(isnan(sqdev))) = 0;
ssdev = sum(sqdev,4);
rmsdev = sqrt(ssdev./repmat(nhi,[1 1 lagcount])); 
hfhisem = (rmsdev./repmat(sqrt(nhi),[1 1 lagcount]));
