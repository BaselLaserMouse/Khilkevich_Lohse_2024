...
% bw 22 mar 2004
% added timing jitter to indexfile
rand('state',0);
repnums  = [6 6 7 7 8 8 9 9 10 10]';
order = [1:OUTFRAMES*2]';
order = repmat(order,length(repnums),1);
reps  = repmat(repnums,1,OUTFRAMES*2);
reps  = reshape(reps',OUTFRAMES*2*length(repnums),1);
order(:,2) = reps;
order = order(randperm(size(order,1)),:);

for ii=1:size(order,1)
  fn=sprintf('image_%.4d.pgm',order(ii,1));
  fprintf(fid,'%s %d\n',fn,order(ii,2));
end
