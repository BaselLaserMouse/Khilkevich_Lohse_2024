function [x,mc,mn,mx]=melbankmbw(p,n,fs,fl,fh,w)
%MELBANKM determine matrix for a mel/erb/bark-spaced filterbank [X,MN,MX]=(P,N,FS,FL,FH,W)
%
% As melbankm but with Nicol's cat-erb spacing available (option 'k')
%
% Inputs:
...
switch wr
    ...
    case 'k'
        pf=(frq2erbcat((b1:b4)*fs/n)-mflh(1))/melinc;
    case 'b'
...
if ~nargout || any(w=='g') % plot idealized filters
    ...
    switch wr
        ...
        case 'k'
            fe=erb2frqcat(me); % defining frequencies
            xg=erb2frqcat(repmat(linspace(0,1,ng),p,1).*repmat(me(3:end)-me(1:end-2),1,ng)+repmat(me(1:end-2),1,ng));
        case 'b'
