function Xstruct = initfit_TrialsphistDesignMat(gg,Xstruct)
...
for T=1:gg.TrialNo 
    % Design matrix for self-coupling filter
    if nh>0
        XspTrials{T}(:,1:nh) = spikefilt(full(double(gg.spsTrial{T}>0)),gg.ihbas);
    end
    
    % Design matrix for cross-coupling filters
    for jcpl = 1:nCoupled
        inds = nh+nh2*(jcpl-1)+1:nh+nh2*jcpl; % column indices
        XspTrials{T}(:,inds) = spikefilt(full(double(gg.sps2Trial{T}(:,jcpl))),gg.ihbas2); % insert into design matrix
    end
end
Xsp=cat(1,XspTrials{:});  %concatenate trials

figure(18888)
imagesc(Xsp)
title('spike (self and coupling design matrix)')
