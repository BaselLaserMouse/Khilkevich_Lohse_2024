...
if strcmp(gg.ktype, 'linear') % standard GLM
   ...
   % if Multitrial == 1
        Xstruct = initfit_TrialstimDesignMat(gg,Stim);  % create design matrix structure concatenated from trials
  %  else
  %      Xstruct = initfit_stimDesignMat(gg,Stim);  % create design matrix structure (single trial)
  %  end
...
%Xstruct = initfit_sphistDesignMat(gg,Xstruct);  % create design matrix structure (single trial)
Xstruct = initfit_TrialsphistDesignMat(gg,Xstruct);  % create design matrix structure concatenated from trials
