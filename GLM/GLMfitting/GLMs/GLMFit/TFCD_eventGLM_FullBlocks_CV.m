...
clear k_baseline Temp T Tcount BaseON_cols Abort_cols EarlyLick_cols Lick_cols Rew_cols k_BaseON k_Abort k_EarlyLick k_EarlyLick k_Lick k_Rew k_Rew
...
researchLambda=0;
cvfit = glmnet(dm.X,y,'poisson',options);
w= cvfit.beta;

if isempty(cvfit.beta) % this means that the model has not converged, try a new lambda search specific to this kfold to see if this helps. This would probably be best to do for all cv, but if I do this for each kfold, it is gong to take forever (it already tkes super long), so using the lambda from the full fit is worth doing for time.
    researchLambda=1;
    clear w cvfit options
    ...
    cvfit = cvglmnet(dm.X,y,'poisson',options,[],[],[],false); % do not do parallel processing as you are already inside a parfor loop
...
%% Plot kernels HOLD THIS OF FOR PARALLEL PROCSSING
% fig = figure(curClu);
% hold on
...
if researchLambda==1
    ...
    y_hat_fit = glmnetPredict(cvfit,dm.X,lambda);
end
