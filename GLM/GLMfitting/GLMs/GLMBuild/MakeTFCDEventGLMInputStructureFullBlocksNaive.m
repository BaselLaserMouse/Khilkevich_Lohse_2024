...
%     end
    ...
    trial(curTrial).duration = round((length(find(BehavData.Raw.TF{SelectTrial}~=0 & ~isnan(BehavData.Raw.TF{SelectTrial})))*0.016666666666666666666)*1000);
        ...
        %% if trial is a miss % for naive mice this is expectd, but trial will continue and bahviour may be happeing (see GLM for trained mice to better understand what is happening here)
        ...
        if sum(isnan(BehavData.Raw.RT(:,SelectTrial)))==3
            trial(curTrial).lick = NaN; % lick that initates the reward (reaction time is now motoin onset from trial start)
            trial(curTrial).Postlick = NaN; % lick that initates the reward (reaction time is now motoin onset from trial start)
        elseif sum(isnan(BehavData.Raw.RT(:,SelectTrial)))==2
            trial(curTrial).lick = BehavData.Raw.RT(1,SelectTrial)*1000; % lick that initates the reward (reaction time is now motoin onset from trial start)
            trial(curTrial).Postlick = BehavData.Raw.RT(1,SelectTrial)*1000; % lick that initates the reward (reaction time is now motoin onset from trial start)
            
        elseif sum(isnan(BehavData.Raw.RT(:,SelectTrial)))==1
            trial(curTrial).lick = BehavData.Raw.RT(1:2,SelectTrial)*1000; % lick that initates the reward (reaction time is now motoin onset from trial start)
            trial(curTrial).Postlick = BehavData.Raw.RT(1:2,SelectTrial)*1000; % lick that initates the reward (reaction time is now motoin onset from trial start)
